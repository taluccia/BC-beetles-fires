---
title: "BRT Drop Prefire Vegetation"
author: "Anna Talucci"
date: "6/18/2021"
...
# Overview
Drop prefire vegetation to assessPeak outbreakseverity variable
## Notes
...
# Data
```{r}
model.data = read.csv("../data/TrainingData/TrainData.csv", header=TRUE, sep = ",", strip.white = TRUE)
```
...
```{r}
save(brt.model1, file = "../outputs/BRTDropNDVI/BRT1_model.rda")
```

```{r eval=FALSE, include=FALSE}
load(file = "../outputs/BRTDropNDVI/BRT1_model.rda")
```
...
dev.print(pdf, "../outputs/BRTDropNDVI/BRT1_plots.pdf")
dev.off()
```
```{r}
gbm.plot.fits(brt.model1)
dev.print(pdf, "../outputs/BRTDropNDVI/BRT1_fits.pdf")
dev.off()
```
...
write.csv(BRT.model1.sum, '../outputs/BRTDropNDVI/BRT1_sum.csv', row.names = F)
BRT.model1.relinf=data.frame(rel.inf, vnames)
write.csv(BRT.model1.relinf, '../outputs/BRTDropNDVI/BRT1_relinf.csv', row.names = F)
```
...
```{r}
set.seed(15) # Set random seed
brt.model2 <- gbm.step(data=data_hi,
    gbm.x = c(1,2,3), 
    gbm.y = 5,
    family = "bernoulli",
    tree.complexity = 5,
...
```{r}
save(brt.model2, file = "../outputs/BRTDropNDVI/BRT2_model.rda")
```

```{r eval=FALSE, include=FALSE}
load(file = "../outputs/BRTDropNDVI/BRT2_model.rda")
```
...
dev.print(pdf, "../outputs/BRTDropNDVI/BRT2_plots.pdf")
dev.off()
```
```{r}
gbm.plot.fits(brt.model2)
dev.print(pdf, "../outputs/BRTDropNDVI/BRT2_fits.pdf")
dev.off()
```
...
```{r}
tot.n=nrow(data_hi)
y.perc=round(nrow(data_hi[data_hi$yvar==1,])/tot.n,2)
...
write.csv(BRT.model2.sum, '../outputs/BRTDropNDVI/BRT2_sum.csv', row.names = F)
BRT.model2.relinf=data.frame(rel.inf, vnames)
write.csv(BRT.model2.relinf, '../outputs/BRTDropNDVI/BRT2_relinf.csv', row.names = F)
```
...
```{r}
set.seed(15) # Set random seed
brt.model3 <- gbm.step(data=data_lo,
    gbm.x = c(1,2,3), 
    gbm.y = 5,
    family = "bernoulli",
    tree.complexity = 5,
...
```{r}
save(brt.model3, file = "../outputs/BRTDropNDVI/BRT3_model.rda")
```

```{r eval=FALSE, include=FALSE}
load(file = "../outputs/BRTDropNDVI/BRT3_model.rda")
```
...
dev.print(pdf, "../outputs/BRTDropNDVI/BRT3_plots.pdf")
dev.off()
```
```{r}
gbm.plot.fits(brt.model3)
dev.print(pdf, "../outputs/BRTDropNDVI/BRT3_fits.pdf")
dev.off()
```
...
```{r}
tot.n=nrow(data_lo)
y.perc=round(nrow(data_lo[data_lo$yvar==1,])/tot.n,2)
...
write.csv(BRT.model3.sum, '../outputs/BRTDropNDVI/BRT3_sum.csv', row.names = F)
BRT.model3.relinf=data.frame(rel.inf, vnames)
write.csv(BRT.model3.relinf, '../outputs/BRTDropNDVI/BRT3_relinf.csv', row.names = F)
```
